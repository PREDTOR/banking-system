...
con1 = mysql.connector.connect(host='localhost', user='root', password='asdf')
#if conic.isconnected():

mycursor=con1.cursor()
mycursor.execute("create database if not exists bank_sys1")
mycursor.execute("use bank_sys1")
mycursor.execute("create table if not exists signup(username varchar(40),password varchar(40))") #here selecting all the contents from the table name entered in code
#coding for project
def signup():
    ...
    print("note: Please do not share your username and password with anyone")
    login()
...
# data2 is for amount table
    ...
    mycursor.execute("create table if not exists account(name varchar(40),account_number int(40),address varchar(50),contact_no varchar(20),total_balance int,account_pin int(20))")
...
def show_info():
    ...
    print("")
    print("")
    print("RK BANK ONLINE PORTAL__________")
    print("RK BANK__________")
    print("___________________________________________________________________________________________________________")
    print("MAKING BANKING EASY FOR EVERYONE___________")
    print("PRESS:")
    print("\t1:SIGNUP\n\n \t2:LOGIN")
    ch = int(input("\n\n\t SIGNUP / LOGIN(1,2):"))
    if ch==1:
    ...
    if ch==2:
        login()
    else: print("WRONG ENTRY TRY AGAIN....")
